    ...
    public static void main(String[] args)
    {
        ...
        while (true)
        {
            ...
            if(!Character.isDigit(line.charAt(0))) {
            	line = "ERROR EN EL FORMATO";
            }else if(peticiones>=10){
            	line = "Reserva fallida: Ha superado el máximo número de reservas";
            }else {
            	line = reserva(line);
            }
            
            DatagramPacket ss= new DatagramPacket(
            		line.getBytes(StandardCharsets.UTF_8),
            		line.getBytes(StandardCharsets.UTF_8).length,
            		receivedDatagram.getSocketAddress());
            try {
				server.send(ss);
			} catch (IOException e) {
